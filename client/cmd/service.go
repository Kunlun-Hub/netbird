...
func init() {
	...
	serviceCmd.PersistentFlags().BoolVar(&captureEnabled, "enable-capture", false, "Enables packet capture via 'cloink debug capture'. To persist, use: cloink service install --enable-capture")
