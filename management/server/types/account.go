...
type Account struct {
	...
	NetworkMapCache             *NetworkMapBuilder `json:"-" gorm:"-"`
	nmapInitOnce                *sync.Once         `json:"-" gorm:"-"`
}
...
// peerIsNameserver returns true if the peer is a nameserver for a nameserver group.
...
func (a *Account) getRulePeers(rule *PolicyRule, postureChecks []string, peerID string, distributionPeers map[string]struct{}, validatedPeersMap map[string]struct{}) []*nbpeer.Peer {
	...
	for _, pID := range a.getResourceRuleSourcePeers(rule) {
		if pID == peerID {
			continue
		}
...
func getDefaultPermit(route *route.Route) []*RouteFirewallRule {
	...
	// Dynamic routes always contain an IPv4 placeholder as destination, so add IPv6 rules too.
	if route.IsDynamic() {
		ruleV6 := rule
...
func (a *Account) GetPeerNetworkResourceFirewallRules(ctx context.Context, peer *nbpeer.Peer, validatedPeersMap map[string]struct{}, routes []*route.Route, resourcePolicies map[string][]*Policy) []*RouteFirewallRule {
	...
	for _, route := range routes {
		...
		distributionPeers := a.getPoliciesSourcePeers(resourceAppliedPolicies)
...
// filterRoutesFromPeersOfSameHAGroup filters and returns routes that don't share the same HA route membership.
func (a *Account) filterRoutesFromPeersOfSameHAGroup(routes []*route.Route, peerMemberships LookupMap) []*route.Route {
	var filteredRoutes []*route.Route
	for _, r := range routes {
		if _, found := peerMemberships[string(r.GetHAUniqueID())]; !found {
...
// filterRoutesByGroups returns routes that have distribution groups in the group's map.
func (a *Account) filterRoutesByGroups(routes []*route.Route, groupListMap LookupMap) []*route.Route {
	var filteredRoutes []*route.Route
	for _, r := range routes {
		for _, groupID := range r.Groups {
			if _, found := groupListMap[groupID]; found {
				filteredRoutes = append(filteredRoutes, r)
				break
			}
		}
...
// getPoliciesSourcePeers collects all unique peers from the sources defined in the given policies.
func (a *Account) getPoliciesSourcePeers(policies []*Policy) map[string]struct{} {
	sourcePeers := make(map[string]struct{})

	for _, policy := range policies {
		for _, rule := range policy.Rules {
			for _, peerID := range a.getResourceRuleSourcePeers(rule) {
...
func (a *Account) getResourceRuleSourcePeers(rule *PolicyRule) []string {
	sourcePeers := make(map[string]struct{})
	if rule.SourceResource.Type == ResourceTypePeer && rule.SourceResource.ID != "" {
		sourcePeers[rule.SourceResource.ID] = struct{}{}
	}
	for _, peerID := range a.getUniquePeerIDsFromGroupsIDs(context.Background(), rule.Sources) {
		sourcePeers[peerID] = struct{}{}
	}
	for _, peerID := range a.getPeerIDsFromUsers(rule.SourceUsers) {
		sourcePeers[peerID] = struct{}{}
	}
	for _, peerID := range a.getPeerIDsFromUserGroups(rule.SourceUserGroups) {
		sourcePeers[peerID] = struct{}{}
	}
