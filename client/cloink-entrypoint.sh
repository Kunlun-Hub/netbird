...
: ${CL_ENTRYPOINT_SERVICE_TIMEOUT:=${NB_ENTRYPOINT_SERVICE_TIMEOUT:-"30"}}
NETBIRD_BIN="${NETBIRD_BIN:-"cloink"}"
export NB_LOG_FILE="${CL_LOG_FILE:-${NB_LOG_FILE:-"console,/var/log/cloink/client.log"}}"
export CL_LOG_FILE="$NB_LOG_FILE"
service_pids=()
...
on_exit() {
  info "Shutting down Cloink daemon..."
...
main() {
  ...
  service_pids+=("$!")
  info "registered new service process 'cloink service run', currently running: ${service_pids[@]@Q}"

  wait_for_daemon_startup "${CL_ENTRYPOINT_SERVICE_TIMEOUT}"
  connect
