...
read_nb_domain() {
  READ_NETBIRD_DOMAIN=""
  echo -n "Enter the domain you want to use for Cloink (e.g. netbird.my-domain.com): " > /dev/stderr
...
read_reverse_proxy_type() {
  ...
  echo "  [6] Traefik (custom HTTPS port, DNS challenge certificates)" > /dev/stderr
  echo "  [7] Traefik (custom HTTPS port, local certificate files)" > /dev/stderr
  echo "" > /dev/stderr
  echo -n "Enter choice [0-7] (default: 0): " > /dev/stderr
  ...
  if [[ ! "$CHOICE" =~ ^[0-7]$ ]]; then
    echo "Invalid choice. Please enter a number between 0 and 7." > /dev/stderr
    read_reverse_proxy_type
    return
  fi
...
read_proxy_docker_network() {
  ...
  echo "If yes, enter the Docker network ${proxy_name} is on (Cloink will join it)." > /dev/stderr
...
read_traefik_https_port() {
  echo "" > /dev/stderr
  echo "Enter the public HTTPS port for Traefik." > /dev/stderr
  echo "This port will be used to access Cloink, for example https://$NETBIRD_DOMAIN:8443." > /dev/stderr
  echo -n "HTTPS port (default: 8443): " > /dev/stderr
  read -r PORT < /dev/tty
  if [[ -z "$PORT" ]]; then
    PORT="8443"
  fi
  if [[ ! "$PORT" =~ ^[0-9]+$ ]] || (( PORT < 1 || PORT > 65535 )); then
    echo "Invalid port. Please enter a number between 1 and 65535." > /dev/stderr
    read_traefik_https_port
    return
  fi
  if [[ "$PORT" == "443" ]]; then
    echo "This option is for deployments that do not use port 443. Enter a different HTTPS port." > /dev/stderr
    read_traefik_https_port
    return
  fi
  echo "$PORT"
...
read_traefik_dns_provider() {
  echo "" > /dev/stderr
  echo "Enter the Traefik/lego DNS provider name." > /dev/stderr
  echo "Examples: cloudflare, alidns, dnspod, route53." > /dev/stderr
  echo -n "DNS provider: " > /dev/stderr
  read -r PROVIDER < /dev/tty
  if [[ -z "$PROVIDER" ]]; then
    echo "DNS provider is required." > /dev/stderr
    read_traefik_dns_provider
    return
  fi
  echo "$PROVIDER"
...
read_traefik_dns_env_name() {
  echo "" > /dev/stderr
  echo "Enter the API token environment variable required by your DNS provider." > /dev/stderr
  echo "Example: Cloudflare uses CF_DNS_API_TOKEN; AliDNS commonly uses ALICLOUD_ACCESS_KEY and ALICLOUD_SECRET_KEY." > /dev/stderr
  echo "If your provider requires multiple variables, edit traefik.environment in docker-compose.yml after installation." > /dev/stderr
  echo -n "Environment variable name: " > /dev/stderr
...
read_traefik_dns_env_value() {
  local env_name="$1"
  echo "" > /dev/stderr
  echo "Enter the value for $env_name. It will be written to docker-compose.yml; protect the file permissions." > /dev/stderr
  echo -n "$env_name: " > /dev/stderr
  read -r ENV_VALUE < /dev/tty
  if [[ -z "$ENV_VALUE" ]]; then
    echo "$env_name is required." > /dev/stderr
...
read_local_certificate_file() {
  ...
  local example="$2"
  echo "" > /dev/stderr
  echo "Enter the local TLS ${label} file path." > /dev/stderr
  echo "Example: ${example}" > /dev/stderr
  echo -n "${label} file path: " > /dev/stderr
  read -r CERT_FILE < /dev/tty
  if [[ -z "$CERT_FILE" ]]; then
    echo "${label} file path is required." > /dev/stderr
    read_local_certificate_file "$label" "$example"
    return
  fi
  if [[ ! -f "$CERT_FILE" ]]; then
    echo "File does not exist: $CERT_FILE" > /dev/stderr
    read_local_certificate_file "$label" "$example"
    return
  fi
  echo "$CERT_FILE"
...
configure_reverse_proxy() {
  ...
  if [[ "$REVERSE_PROXY_TYPE" == "6" ]]; then
    if [[ "$NETBIRD_DOMAIN" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ || "$NETBIRD_DOMAIN" == *:* ]]; then
      echo "Traefik DNS challenge requires a domain name; IP addresses and use-ip mode are not supported." > /dev/stderr
      exit 1
    fi
    TRAEFIK_ACME_EMAIL=$(read_traefik_acme_email)
  ...
  if [[ "$REVERSE_PROXY_TYPE" == "7" ]]; then
    ...
    TRAEFIK_LOCAL_CERT_FILE=$(read_local_certificate_file "certificate" "./cret/saas.4w.ink.pem")
    TRAEFIK_LOCAL_KEY_FILE=$(read_local_certificate_file "private key" "./cret/saas.4w.ink.key")
    ENABLE_PROXY=$(read_enable_proxy)
    if [[ "$ENABLE_PROXY" == "true" ]]; then
      ENABLE_CROWDSEC=$(read_enable_crowdsec)
    fi
...
print_builtin_traefik_instructions() {
  echo ""
  echo "$MSG_SEPARATOR"
  echo "  CLOINK SETUP COMPLETE"
  echo "$MSG_SEPARATOR"
  echo ""
  echo "You can access the Cloink dashboard at:"
  echo "  $NETBIRD_PUBLIC_URL"
  echo ""
  echo "Follow the onboarding steps to set up your Cloink instance."
  echo ""
  if [[ "$TRAEFIK_ACME_CHALLENGE" == "dns" ]]; then
    echo "Traefik is handling TLS certificates automatically via Let's Encrypt DNS-01 challenge."
    echo "DNS provider: $TRAEFIK_DNS_PROVIDER"
  elif [[ "$TRAEFIK_ACME_CHALLENGE" == "local" ]]; then
    echo "Traefik is using the local TLS certificate files you provided."
    echo "Certificate file: $TRAEFIK_LOCAL_CERT_FILE"
    echo "Private key file: $TRAEFIK_LOCAL_KEY_FILE"
  else
    echo "Traefik is handling TLS certificates automatically via Let's Encrypt."
  fi
  if [[ "$TRAEFIK_ACME_CHALLENGE" == "local" ]]; then
    echo "If you see certificate warnings, verify that the domain, certificate chain, and private key match."
  else
    echo "If you see certificate warnings, wait a moment for certificate issuance to complete."
  fi
  echo ""
  echo "Open ports:"
  echo "  - $TRAEFIK_HTTPS_PORT/tcp   (HTTPS - all Cloink services)"
...
print_nginx_instructions() {
  ...
  echo "$MSG_SEPARATOR"
  echo ""
  echo "Generated: cloink-nginx.conf"
  echo ""
  ...
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    ...
    echo "  2. Edit cloink-nginx.conf and update SSL certificate paths"
    ...
    echo "  2. Edit cloink-nginx.conf and update certificate paths"
...
print_external_caddy_instructions() {
  ...
  echo "$MSG_SEPARATOR"
  echo ""
  echo "Generated: caddyfile-cloink.txt"
  echo ""
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    echo "Cloink containers have joined the '$EXTERNAL_PROXY_NETWORK' Docker network."
    echo "The config uses container names for upstream servers."
    echo ""
    echo "$MSG_NEXT_STEPS"
    echo "  1. Add the contents of caddyfile-cloink.txt to your Caddyfile"
    echo "  2. Reload Caddy"
  else
    echo "$MSG_NEXT_STEPS"
    echo "  1. Add the contents of caddyfile-cloink.txt to your Caddyfile"
...
print_manual_instructions() {
  ...
  echo ""
  echo "  Native gRPC (signal + management + flow log upload + reverse proxy control):"
