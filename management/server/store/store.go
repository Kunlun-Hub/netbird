...
type Store interface {
	...
	GetAccountNetworkTrafficClientGroups(ctx context.Context, lockStrength LockingStrength, accountID string, filter networktraffic.Filter) ([]networktraffic.ClientGroup, int64, error)
	GetAccountNetworkTrafficGroupFlows(ctx context.Context, lockStrength LockingStrength, accountID string, filter networktraffic.Filter) ([]*networktraffic.Event, int64, error)
	GetAccountNetworkTrafficSummary(ctx context.Context, accountID string, filter networktraffic.Filter, bucketSeconds int) ([]networktraffic.SummaryPoint, error)
...
func getMigrationsPreAuto(ctx context.Context) []migrationFunc {
	return []migrationFunc{
		...
		func(db *gorm.DB) error {
			return migration.MigrateNewField[types.User](ctx, db, "user_groups", "[]")
		},
		func(db *gorm.DB) error {
