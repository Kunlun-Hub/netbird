	...
	if delta.RebuildRoutesView {
		...
		if routesView := b.cache.peerRoutes[peerID]; routesView != nil {
			if len(delta.RemoveRoutes) > 0 {
				...
				routesView.InheritedRouteIDs = slices.DeleteFunc(routesView.InheritedRouteIDs, func(routeID route.ID) bool {
					return slices.Contains(delta.RemoveRoutes, routeID)
				})
				routesView.OwnRouteIDs = slices.DeleteFunc(routesView.OwnRouteIDs, func(routeID route.ID) bool {
					return slices.Contains(delta.RemoveRoutes, routeID)
				})
			}
			...
			if len(delta.AddRoutes) > 0 {
				for _, routeID := range delta.AddRoutes {
					if !slices.Contains(routesView.NetworkResourceIDs, routeID) &&
						!slices.Contains(routesView.InheritedRouteIDs, routeID) &&
						!slices.Contains(routesView.OwnRouteIDs, routeID) {
						routesView.NetworkResourceIDs = append(routesView.NetworkResourceIDs, routeID)
					}
				}
			}
...
// OnRouteAdded handles incremental update when a route is added
func (b *NetworkMapBuilder) OnRouteAdded(account *Account, r *route.Route) ([]string, error) {
	b.cache.mu.Lock()
	defer b.cache.mu.Unlock()
...
// OnRouteUpdated handles incremental update when a route is updated
func (b *NetworkMapBuilder) OnRouteUpdated(account *Account, oldRoute, newRoute *route.Route) ([]string, error) {
	b.cache.mu.Lock()
	defer b.cache.mu.Unlock()
...
// OnRouteDeleted handles incremental update when a route is deleted
func (b *NetworkMapBuilder) OnRouteDeleted(account *Account, r *route.Route) ([]string, error) {
	b.cache.mu.Lock()
	defer b.cache.mu.Unlock()
...
// updateRouteGroupIndex updates the group to route index
func (b *NetworkMapBuilder) updateRouteGroupIndex(r *route.Route) {
	for _, groupID := range r.Groups {
		if !slices.Contains(b.cache.groupToRoutes[groupID], r) {
			b.cache.groupToRoutes[groupID] = append(b.cache.groupToRoutes[groupID], r)
		}
...
// removeRouteFromIndexes removes route from all indexes
func (b *NetworkMapBuilder) removeRouteFromIndexes(r *route.Route) {
	// Remove from groupToRoutes
	for _, groupID := range r.Groups {
		b.cache.groupToRoutes[groupID] = slices.DeleteFunc(b.cache.groupToRoutes[groupID], func(rt *route.Route) bool {
			return rt.ID == r.ID
		})
		if len(b.cache.groupToRoutes[groupID]) == 0 {
	...
	if r.Peer != "" {
		...
		if len(b.cache.peerToRoutes[r.Peer]) == 0 {
			delete(b.cache.peerToRoutes, r.Peer)
		}
	}
}
