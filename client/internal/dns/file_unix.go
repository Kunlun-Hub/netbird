...
const (
	fileGeneratedResolvConfContentHeader         = "# Generated by Cloink"
	fileLegacyGeneratedResolvConfContentHeader   = "# Generated by NetBird"
...
func (f *fileConfigurator) updateConfig(nbSearchDomains []string, nbNameserverIP netip.Addr, cfg *resolvConf, stateManager *statemanager.Manager) error {
	...
	log.Infof("created a Cloink managed %s file with the DNS settings. Added %d search domains. Search list: %s", defaultResolvConfPath, len(searchDomainList), searchDomainList)
...
func (f *fileConfigurator) restoreUncleanShutdownDNS(storedDNSAddress netip.Addr) error {
	...
	// current address is still Cloink's non-available DNS address -> restore
	currentDNSAddress := resolvConf.nameServers[0]
	if currentDNSAddress == storedDNSAddress {
		return restoreResolvConfFile()
	}

	log.Infof("restoring unclean shutdown: first current nameserver differs from saved nameserver pre-cloink: %s (current) vs %s (stored): not restoring", currentDNSAddress, storedDNSAddress)
	return nil
}
